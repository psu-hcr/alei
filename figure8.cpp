...
class figureight{
	...
	sac<KoopSys<iiwaBasis2>, errorcost<KoopSys<iiwaBasis2>>> *sacsysKPointer;
			alk<KoopSys<iiwaBasis2>,fishcost<KoopSys<iiwaBasis2>,errorcost<KoopSys<iiwaBasis2>>>,sac<KoopSys<iiwaBasis2>,errorcost<KoopSys<iiwaBasis2>>>> *ALKpointer;
		
	public:
	figureight(ros::NodeHandle *_nh, KoopSys<iiwaBasis2> *_systKPointer, iiwaBasis2 *_iiwaBasisPointer, sac<KoopSys<iiwaBasis2>, errorcost<KoopSys<iiwaBasis2>>> *_sacsysKPointer, alk<KoopSys<iiwaBasis2>,fishcost<KoopSys<iiwaBasis2>,errorcost<KoopSys<iiwaBasis2>>>,sac<KoopSys<iiwaBasis2>,errorcost<KoopSys<iiwaBasis2>>>> *_alKpointer){
		
		nh = _nh;
		systKPointer = _systKPointer;
		iiwaBasisPointer = _iiwaBasisPointer;
		sacsysKPointer = _sacsysKPointer;
		ALKpointer = _alKpointer;
		Xcurr = arma::zeros(21);
		Ucurr = arma::zeros(7);
...
int main(int argc, char **argv){
	...
	errorcost<KoopSys<iiwaBasis2>> costK (Qk,R,xdk,&systK);
	
	// define fisher cost
	arma::vec noisecov = 1.0*arma::ones(iiwaBasis.xdim);
	fishcost<KoopSys<iiwaBasis2>,errorcost<KoopSys<iiwaBasis2>>> costFI (&systK,&costK,noisecov);
	
	// define sac controllor
	sac<KoopSys<iiwaBasis2>,errorcost<KoopSys<iiwaBasis2>>> sacsysK (&systK,&costK,t_calc,T,umax,unom);
	
	// define active learning class
	arma::mat Rtil = 1.*arma::eye(systK.Ucurr.n_rows,systK.Ucurr.n_rows);
alk<KoopSys<iiwaBasis2>,fishcost<KoopSys<iiwaBasis2>,errorcost<KoopSys<iiwaBasis2>>>,sac<KoopSys<iiwaBasis2>,errorcost<KoopSys<iiwaBasis2>>>> ALK(&systK,&costFI,&sacsysK,T,umax,Rtil);
	
	// multithread for each CPU core
	ros::MultiThreadedSpinner spinner(0);
	
	figureight feight = figureight(&nh, &systK, &iiwaBasis, &sacsysK, &ALK);
