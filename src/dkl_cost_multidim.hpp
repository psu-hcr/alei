#ifndef dklcost_multidim_MULTIDIM_HPP
#define dklcost_multidim_MULTIDIM_HPP
#include<armadillo>
#include<math.h>
#include<omp.h>
using namespace std;
...
class dklcost_multidim {
  system* sys;
  double L1,L2,L3, L4, L5, L6, L7, T;
  int X1,X2, X3, X4, X5, X6, X7;//index of relavant dimensions in xvector
  arma::uvec X_DKL;
  ...
  public:
    ...
    dklcost_multidim(double _Q, arma::mat _R,int _K,arma::mat _sigma, int _X1,int _X2, int _X3, int _X4,int _X5, int _X6, int _X7, std::function<double(const arma::vec&)> _phid,
	double _L1,double _L2, double _L3, double _L4, double _L5, double _L6, double _L7, double _T,system *_sys){
		Q=_Q; R=_R; sys=_sys; K = _K; phid = _phid; T=_T; // initialize with Q, R, sys, phid, and the domain
		X1 = _X1; X2=_X2; X3=_X3; X4 = _X4; X5=_X5; X6=_X6; X7=_X7;
		L1 = _L1; L2 = _L2; L3 = _L3; L4 = _L4; L5 = _L5; L6 = _L6; L7 = _L7;
		X_DKL<<X1<<X2<<X3<<X4<<X5<<X6<<X7; sigma=_sigma;
		T_index = T/sys->dt;
...
template<class system> double dklcost_multidim<system>::l (const arma::vec& x,const arma::vec& u,double ti){
	...
	Qtemp(X4,X4)= pow(xproj(X4)/(L4+(0.1*L1)),8);
    Qtemp(X5,X5) = pow(xproj(X5)/(L5+(0.1*L2)),8);
	Qtemp(X6,X6) = pow(xproj(X6)/(L6+(0.1*L3)),8);
	Qtemp(X7,X7) = pow(xproj(X7)/(L7+(0.1*L3)),8);
    return arma::as_scalar((xproj.t()*Qtemp*xproj+u.t()*R*u)/2);
}

template<class system> arma::vec dklcost_multidim<system>::dldx (const arma::vec&x, const arma::vec& u, double ti){
	arma::vec xproj = sys->proj_func(x);
...
template<class system> double dklcost_multidim<system>::calc_cost (const arma::mat& x,const arma::mat& u){
  double J1 = 0.,Jtemp;
  arma::mat xjoined;
...
template<class system> void dklcost_multidim<system>::qs_disc(const arma::mat& x){//double start_time = omp_get_wtime();
...
template<class system> void dklcost_multidim<system>::resample(){
  //Choose K samples over the domain [[-L1,L1],[-L2,L2]]
  arma::vec domainsize = {2.*L1, 2.*L2, 2.*L3, 2.*L4, 2.*L5, 2.*L6, 2.*L7};//{2*L1,2*L2};
...
template<class system> void dklcost_multidim<system>::xmemory(const arma::vec& x){
  xpast.col(t_now)= x;
  t_now++;
  resample();
}
